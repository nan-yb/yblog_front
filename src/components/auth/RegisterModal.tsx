...
interface Props {
  readonly onSignUp : (email : string , password : string , nickName : string , company : string) => void;
}

const RegisterModal = ( { onSignUp } : Props)  => {
  ...
  const handleChangeCompany = useCallback((e:React.ChangeEvent<HTMLSelectElement>) =>{
  ...
  const fetchHotels = async () => {
    try {
      // 요청 처음에 초기화
      setTotalCompany([]);
      const data : any = await api.fetchCompanyList();
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // if(codeDetail) {
      onSignUp(email , password , nickName , company);
    // }
  };
                    ...
                    <div>
                      <div className="md:flex md:items-center mb-6">
                        ...
                        <div className="relative mt-1 mb-2">
                          ...
                          >
                            <option>콘텐츠를 선택하세요.</option>
                            {totalCompany && totalCompany.map((company : any) => (
